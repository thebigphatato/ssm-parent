 ...
 *      优点：代码量小
 *      缺点：封装太多。排错不容易
 * 编程式：通过代码等方式，告诉框架，我要做什么，框架会帮我做什么
 *      优点：排错容易
 *      缺点：代码量多
 *
 * AOP的使用场景
 * 1、日志记录：
 * 在不修改业务代码情况下，为方法调用添加日志记录功能。这有助于跟踪方法调用的时间、参数、返回值、以及异常信息等。
 * 2、事务管理：
 ...
 * 4、性能监控：(这个往往使用专业的框架来，而不是用AOP)
 * 对方法的执行时间进行监控，以评估系统的性能瓶颈。AOP可以帮助在不修改业务代码的情况下，为方法调用添加性能监控的逻辑。
 * 5、异常处理：
 * 集中处理业务逻辑中可能抛出的异常，并进行统一的日志记录或错误处理。通过AOP。可以为方法调用添加异常捕获和处理的逻辑
 * 6、缓存管理：
 * 在方法调用前后添加缓存逻辑，以提高系统的响应速度和吞吐量。AOP可以帮助实现缓存的自动加载、更新和失效等逻辑。
...
public class AroundAspect {
     ...
     * Object: 返回值
     * 参数：必须有一个ProceedingJoinPoint:可以继续推进的切点
     */
...
//        args[1] = 5;// 这个地方可以该参数值
        ...
        try {
            ...
            proceed = pjp.proceed(args);// 继续执行目标方法;相当于动态代理中的反射 method.invoke();
            // 返回
            System.out.println("环绕 - 返回通知：" + proceed);
        }catch(Throwable e) {
            // 异常
            System.out.println("环绕 - 异常通知" + e.getMessage());
            throw new RuntimeException(e); // 让别人继续感知
        }finally {
            // 后置
